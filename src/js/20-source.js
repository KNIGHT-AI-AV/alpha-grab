...
/* Which typefaces does this graphic ACTUALLY put on screen?

   Measured on a live Flowics lower third: it renders exactly ONE webfont,
   HelveticaNeueLTStd. We were embedding fourteen faces across thirty files and
   892 KB — including three complete icon-font families the graphic never shows
   a glyph of. That cost 29 relay round trips on every single download, and then
   the rasteriser had to parse all 892 KB of base64 again to draw one frame.
   Together they were 11 of the 22 seconds between pressing Download and getting
   a file.

   Only families reached by text that is actually painted count. Hidden icon
   spans have no layout box, so they fall out on their own. */
function usedFamilies(doc){
  const out = new Set();
  const win = doc.defaultView;
  if (!doc.body || !win) return out;
  ...
  for (let n = walk.nextNode(); n; n = walk.nextNode()) {
    ...
    let r;
    try { const rg = doc.createRange(); rg.selectNodeContents(n); r = rg.getBoundingClientRect(); } catch (_) { continue; }
    if (r.width < 2 || r.height < 2) continue;
    for (const f of String(cs.fontFamily || '').split(','))
      out.add(f.trim().replace(/^["']|["']$/g, '').toLowerCase());
  }
  return out;
}

/* Font files do not change between two downloads of the same graphic, and the
   operator's loop is downloads of the same graphic. Keyed on the base URL and
   the exact set of families wanted, so a graphic that swaps to a different
   typeface still re-reads rather than serving the old one. */
const FONT_CACHE = new Map();

async function collectFontFaces(doc, baseUrl, note){
  const want = usedFamilies(doc);
  const key = (baseUrl || '') + '|' + [...want].sort().join(',');
  if (FONT_CACHE.has(key)) return FONT_CACHE.get(key);

  let out = '';
  const ctx = { ok: new Set(), failed: [], want };
  for (const sheet of Array.from(doc.styleSheets || [])) {
        ...
        try {
          ...
          if (r && r.ok) {
            for (const block of fontFaceBlocks(await r.text())) {
              if (!wanted(ctx, block)) { recovered++; continue; }
    ...
    for (const rule of Array.from(rules || [])) {
      ...
      if (!wanted(ctx, rule.cssText)) continue;
  ...
  /* Unbounded growth is not a risk worth a whole eviction policy here — one
     entry per graphic per session — but a runaway page should not be able to
     fill memory either. */
  if (FONT_CACHE.size > 24) FONT_CACHE.delete(FONT_CACHE.keys().next().value);
  return out;
}

/* A face is wanted when something on screen asks for its family. An empty want
   set means we could not read the document, in which case embed everything
   rather than silently stripping the typography. */
function wanted(ctx, cssText){
  if (!ctx || !ctx.want || !ctx.want.size) return true;
  return ctx.want.has(fontFamilyOf(cssText));
}
